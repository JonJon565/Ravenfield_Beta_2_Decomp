...
namespace UnityStandardAssets.Characters.FirstPerson
{
	...
	public class RigidbodyFirstPersonController : MonoBehaviour
	{
		...
		{
			RaycastHit hitInfo;
			if (Physics.SphereCast(base.transform.position, m_Capsule.radius, Vector3.down, out hitInfo, m_Capsule.height / 2f - m_Capsule.radius + advancedSettings.stickToGroundHelperDistance) && Mathf.Abs(Vector3.Angle(hitInfo.normal, Vector3.up)) < 85f)
			{
		...
		private void GroundCheck()
		{
			...
			RaycastHit hitInfo;
			if (Physics.SphereCast(base.transform.position, m_Capsule.radius, Vector3.down, out hitInfo, m_Capsule.height / 2f - m_Capsule.radius + advancedSettings.groundCheckDistance))
			{
				m_IsGrounded = true;
